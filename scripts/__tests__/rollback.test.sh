...
# Must support target-argument. Kontrak berubah 2026-09 (GHCR per-SHA):
# bukan lagi "steps-back" numerik tapi SHA tag spesifik — image per-SHA abadi
# di registry, jadi memilih target eksplisit lebih aman daripada menghitung mundur.
if ! grep -qE 'TARGET|github\.sha|sha' "$SCRIPT"; then
  echo "❌ $SCRIPT missing SHA-target argument"
  exit 1
fi
