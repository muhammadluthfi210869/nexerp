...
export default function FillingTerminalV4() {
  ...
  if (isLoading) return <div className="h-screen bg-white flex items-center justify-center text-gray-900 font-black italic">INITIALIZING FILLING TERMINAL...</div>;
  if (!activeSchedule) return <div className="h-screen bg-white flex items-center justify-center text-gray-900 font-black italic">NO ACTIVE FILLING SCHEDULES.</div>;
  ...
  return (
    <DashboardShell title="Terminal" titleAccent="Filling V.4" subtitle={`Batch: ${activeSchedule.scheduleNumber}`}>
         ...
         <div className="flex items-center gap-6">
            <div>
               <h1 className="text-xl font-black italic tracking-tighter uppercase flex items-center gap-3">
                  Terminal <span className="text-indigo-600">Filling</span> V.4
                  <div className="px-2 py-0.5 bg-indigo-500/10 border border-indigo-500/20 rounded text-[8px] text-indigo-600 font-black tracking-widest uppercase">Precision Stage</div>
               </h1>
               <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em]">Batch: {activeSchedule.scheduleNumber}</p>
            </div>
         </div>

         <div className="flex items-center gap-8">
            <div className="text-right">
               <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Target PCS</p>
               <p className="text-sm font-black italic text-gray-900 uppercase tracking-tight">{activeSchedule.targetQty.toLocaleString()}</p>
            </div>
            <div className="h-10 w-px bg-gray-100" />
            <div className="text-right">
               <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Real-time Clock</p>
               <p className="text-sm font-black italic text-indigo-600 tracking-tight tabular-nums">
      ...
      <main className="flex-1 flex overflow-hidden">
        <section className="w-[450px] border-r border-gray-200 bg-gray-50 overflow-y-auto p-10 space-y-12">
            <div>
               <div className="flex items-center gap-4 mb-6">
                  <div className="p-3 bg-gray-100 rounded-2xl"><Settings className="w-5 h-5 text-indigo-600" /></div>
            ...
            <div>
               <div className="flex items-center gap-4 mb-6">
                  <div className="p-3 bg-gray-100 rounded-2xl"><PackageCheck className="w-5 h-5 text-indigo-600" /></div>
        ...
        <section className="flex-1 bg-white p-12 flex flex-col relative">
           {isScanning && (
              <div className="absolute inset-0 z-[100] bg-white flex flex-col items-center justify-center p-20 animate-in fade-in duration-500">
                 ...
                 <div className="w-full max-w-2xl aspect-square border-4 border-indigo-500/20 rounded-[24px] relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent via-indigo-500/5 to-transparent animate-scan" />
                    <div className="absolute inset-20 border-2 border-dashed border-indigo-500/40 rounded-[24px] flex items-center justify-center">
                       <QrCode className="w-40 h-40 text-indigo-500/20" />
                    </div>
                 </div>
                 <h3 className="mt-12 text-3xl font-black italic tracking-tighter uppercase text-indigo-600">Scan Component Package</h3>
                 <p className="mt-4 text-gray-500 font-bold uppercase tracking-widest text-xs">Awaiting: {activeComponent?.material?.name}</p>
                 <div className="mt-8 w-full max-w-md space-y-4">
                    ...
                    <button
                      ...
                      className="w-full h-14 bg-indigo-600 text-white font-black uppercase rounded-xl text-xs tracking-wider hover:bg-indigo-600/90 transition-all"
                    >
                      Confirm & Submit
                    </button>
                 </div>
           ...
           <div className="flex-1 flex flex-col items-center justify-center space-y-12">
              ...
              >
                 <div className="relative z-10 flex flex-col items-center text-center space-y-10">
                    <div className={cn("w-32 h-32 rounded-[24px] flex items-center justify-center shadow-2xl transition-all duration-700", activeComponent ? "bg-indigo-500" : "bg-emerald-500")}>
                 ...
                 {blockingError && (
                    <div className="absolute inset-0 z-50 bg-rose-600 flex flex-col items-center justify-center p-12 text-center animate-in fade-in duration-300">
                       ...
                       <button 
                         ...
                         className="mt-12 px-10 py-4 bg-white text-gray-900 font-black uppercase rounded-2xl hover:bg-gray-100 hover:text-gray-900 transition-all"
                       >
                          Tutup & Perbaiki
                       </button>
                    </div>
           ...
           <footer className="h-40 border-t border-gray-200 flex items-center justify-between px-12 bg-gray-50 -mx-12 -mb-12">
              <div className="flex gap-12">
                 <div className="text-left">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Efficiency Rate</p>
              ...
              <button 
                ...
                className="h-24 px-16 rounded-[24px] bg-rose-600 text-white font-black italic uppercase text-lg flex items-center gap-4 transition-all hover:scale-105 active:scale-95 shadow-xl"
              >
              ...
              <button 
                ...
                className="h-24 px-16 rounded-[24px] bg-indigo-500 text-white font-black italic uppercase text-lg flex items-center gap-4 transition-all hover:scale-105 active:scale-95 shadow-xl"
              >
...
function SpecItem({ label, value }: any) {
   return (
      <div className="flex justify-between items-center p-5 rounded-2xl bg-gray-50 border border-gray-200">
         <span className="text-[9px] font-black uppercase text-gray-500 tracking-widest">{label}</span>
         <span className="text-sm font-black italic text-gray-900 uppercase">{value}</span>
      </div>
   );
}
...
function ComponentItem({ name, target, status }: any) {
   return (
      <div className={cn("p-5 rounded-xl border transition-all", status === 'active' ? "bg-indigo-500/5 border-indigo-500/20" : status === 'completed' ? "bg-emerald-500/5 border-emerald-500/20" : "bg-gray-50 border-gray-200")}>
         <div className="flex items-center justify-between mb-2">
            <span className={cn("text-[8px] font-black uppercase tracking-widest", status === 'active' ? "text-indigo-600" : status === 'completed' ? "text-emerald-500" : "text-gray-400")}>{status.toUpperCase()}</span>
            {status === 'completed' && <CheckCircle2 className="w-3 h-3 text-emerald-500" />}
         </div>
         <h4 className={cn("text-xs font-black italic mb-1", status === 'pending' ? "text-gray-500" : "text-gray-900")}>{name}</h4>
         <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tight">Target: {target}</p>
      </div>
   );
}
...
function MetricInput({ label, value, unit, status, onClick }: any) {
   return (
      <div onClick={onClick} className={cn("flex flex-col items-center space-y-4 group", onClick && "cursor-pointer")}>
         <span className="text-[9px] font-black uppercase tracking-[0.4em] text-gray-500">{label}</span>
         <div className={cn("w-full h-28 rounded-2xl bg-gray-100 border flex flex-col items-center justify-center relative overflow-hidden transition-all", status === 'active' ? "border-indigo-500/40" : status === 'warning' ? "border-rose-500/40" : "border-gray-200")}>
            <span className="text-2xl font-black italic text-gray-900 tracking-tighter">{value}</span>
            <span className="text-[10px] font-black text-gray-400 ml-2">{unit}</span>
         </div>
      </div>
   );
}
