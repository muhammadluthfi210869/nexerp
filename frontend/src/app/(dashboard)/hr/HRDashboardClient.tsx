...
export default function HRDashboardClient() {
  ...
  if (loading) {
    return (
      <div className="p-6 bg-base min-h-screen flex items-center justify-center">
  ...
  return (
    <div className="p-6 bg-base min-h-screen space-y-12">
      {/* HEADER SECTION */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <KpiCard
          ...
          value={executive?.budgetSavings || "N/A"}
          targetPct={parseFloat(executive?.budgetSavings) > 0 ? 100 : 0}
          icon={<Wallet />}
        />
        <KpiCard
          label="Hiring Speed"
          value={executive?.hiringSpeed || "N/A"}
          targetPct={parseInt(executive?.hiringSpeed) <= 30 ? 100 : Math.max(0, 100 - (parseInt(executive?.hiringSpeed) - 30) * 3)}
          icon={<UserPlus />}
        />
        <KpiCard
          label="Stability"
          value={`${executive?.stabilityIndex || 0}%`}
          targetPct={executive?.stabilityIndex || 0}
          icon={<Heart />}
        />
        <KpiCard
          label="Avg KPI"
          value={`${executive?.avgKpi || 0}%`}
          targetPct={executive?.avgKpi || 0}
          icon={<Target />}
        />
        <KpiCard
          label="Workload"
          value={executive?.workload ?? "N/A"}
          targetPct={50}
          icon={<Briefcase />}
        />
      </div>
