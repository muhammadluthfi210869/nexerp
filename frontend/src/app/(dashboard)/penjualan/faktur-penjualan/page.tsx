          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <th className="py-3 px-4">REFERENSI SO</th>
                <th className="py-3 px-4 text-right">TAGIHAN NETT</th>
                <th className="py-3 px-4 text-right">TERBAYAR / SISA</th>
                <th className="py-3 px-4 text-center">STATUS BAYAR</th>
                <th className="py-3 px-4 text-center">AR GATEKEEPER (DO)</th>
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-3.5 px-4">
                        <DnaCell.Text primary={inv.invoiceNumber} secondary={`Tgl: ${inv.invoiceDate}`} />
                      </td>
                      <td className="py-3.5 px-4">
                        <DnaCell.Avatar name={inv.customerName} subtext={inv.brandName} />
                      </td>
                      <td className="py-3.5 px-4">
                        <span className="font-mono text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-100">
                          {inv.soNumber}
                        </span>
                        <p className="text-[11px] text-slate-400 mt-0.5">PIC: {inv.picBusDev}</p>
                      </td>
                      <td className="py-3.5 px-4 text-right">
                        <p className="font-bold text-slate-900">
                          Rp {inv.grandTotal.toLocaleString("id-ID")}
                        </p>
                        {inv.downPaymentOffset > 0 && (
                          <p className="text-[11px] text-emerald-600 font-medium">
                            Offset DP: -Rp {inv.downPaymentOffset.toLocaleString("id-ID")}
                          </p>
                        )}
                      </td>
                      <td className="py-3.5 px-4 text-right">
                        <p className="font-bold text-slate-700 text-xs">
                          Masuk: Rp {inv.paidAmount.toLocaleString("id-ID")}
                        </p>
                        <p
                        ...
                        >
                          {remaining > 0 ? `Sisa: Rp ${remaining.toLocaleString("id-ID")}` : "LUNAS"}
                        </p>
                      </td>
