...
export default function WarehouseDashboard() {
  ...
  return (
    <DashboardShell
      title="Warehouse"
      titleAccent="Portal v1.0"
      subtitle="Material Requisition (FKP) Verification & Stock Control"
    >
      {(() => {
        const lowStock = materials?.filter(m => m.stock_qty < m.min_stock).length || 0;
        return (
          <div className="grid grid-cols-3 gap-8 mb-6">
            <KpiCard label="Total SKUs" value={String(materials?.length || 0)} targetPct={50} icon={<Package />} />
            <KpiCard
              label="Low Stock"
              value={String(lowStock)}
              targetPct={lowStock === 0 ? 100 : 0}
              icon={<AlertTriangle />}
            />
            <KpiCard
              label="Pending Requisitions"
              value={String(pendingPlans.length)}
              targetPct={pendingPlans.length > 0 ? 50 : 100}
              icon={<ClipboardList />}
            />
          </div>
        );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 <TableBody>
                    {materials?.map(m => (
                      <TableRow key={m.id} className="border-gray-200 hover:bg-gray-50">
                        <TableCell>
                            <p className="text-xs font-bold text-gray-900 uppercase">{m.name}</p>
                           <p className="text-[10px] font-sans text-zinc-500 italic uppercase tracking-tighter">{m.id.split('-')[0]}</p>
                        </TableCell>
                        <TableCell>
                           <Badge variant="outline" className="text-[8px] bg-white text-zinc-500 border-gray-200 rounded-none px-2 py-0">
                               {m.type}
                           </Badge>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right font-sans font-bold text-sm text-gray-700">
        ...
        <Card className="lg:col-span-1 border-gray-200 bg-white h-fit">
          <CardHeader>
             <CardTitle className="text-sm font-black text-gray-600 uppercase tracking-tight flex items-center">
          ...
          <CardContent className="space-y-4">
             {pendingPlans.map(plan => (
               <div 
                 key={plan.id}
                 className="p-4 border border-gray-200 bg-black/40 rounded-sm hover:border-emerald-500/50 transition-all cursor-pointer group"
                 onClick={() => { setSelectedPlan(plan); setIsFKPWizardOpen(true); }}
               >
                 <div className="flex justify-between items-start mb-2">
                     <p className="font-sans text-xs text-gray-900 uppercase group-hover:text-emerald-400 font-bold">{plan.batch_no}</p>
             ...
             {pendingPlans.length === 0 && (
                <div className="py-20 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-none bg-gray-50">
                   <ShieldCheck className="h-10 w-10 text-gray-400 mb-2 opacity-20" />
                   <p className="text-[10px] font-black uppercase text-gray-400 tracking-tight italic font-sans text-center px-4">All requisitions cleared. Warehouse idle.</p>
                </div>
             )}
          </CardContent>
        </Card>
      ...
      <Dialog open={isFKPWizardOpen} onOpenChange={setIsFKPWizardOpen}>
        <DialogContent className="bg-white border-gray-200 text-gray-900 sm:max-w-[700px]">
          <DialogHeader>
              <DialogTitle className="flex items-center text-3xl font-black italic uppercase tracking-tighter text-gray-900">
          ...
          {selectedPlan && (
            <div className="space-y-6 py-4">
               <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-gray-50 border border-gray-200">
                     <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Batch Registry</p>
                      <p className="text-sm font-black text-gray-900 uppercase">{selectedPlan.batch_no}</p>
                  </div>
                  <div className="p-3 bg-gray-50 border border-gray-200">
                     <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Production Client</p>
                      <p className="text-sm font-black text-gray-900 uppercase">{selectedPlan.so?.lead?.client_name}</p>
                  </div>
               </div>
                        ...
                        {selectedPlan.requisitions.map(req => {
                          ...
                          return (
                             <TableRow key={req.id} className="border-gray-200">
                                 <TableCell className="text-xs uppercase font-bold text-gray-900">{req.material.name}</TableCell>
                                <TableCell className="font-sans text-gray-700 text-xs">{Number(req.qty_requested).toLocaleString()} {req.material.unit}</TableCell>
               ...
               <DialogFooter>
                  <Button 
                    variant="outline" 
                    className="border-gray-200 text-zinc-500 hover:bg-gray-100"
