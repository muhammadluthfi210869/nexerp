...
export default function WarehouseHubPage() {
 const [selectedItem, setSelectedItem] = useState<MaterialCatalogItem | null>(
 null,
 );
 ...
 const price =
 item.valuations?.[0]?.movingAveragePrice || Number(item.unitPrice);
 return acc + Number(item.stockQty) * price;
 }, 0) || 0;

 const criticalItems =
 catalog?.filter((item) => Number(item.stockQty) <= Number(item.minLevel))
 .length || 0;
 ...
 {/* MAIN GRID */}
 <TableWrapper className="shadow-slate-200/50">
 ...
 const hpp =
 item.valuations?.[0]?.movingAveragePrice ||
 Number(item.unitPrice);
 const isCritical =
 Number(item.stockQty) <= Number(item.minLevel);
 ...
 <div>
 <p className="font-black text-slate-900 tracking-tight text-sm uppercase leading-tight">
 {item.name}
 </p>
 ...
 <span
 className={cn(
 "font-black text-base tracking-tight tabular-nums",
 ...
 {selectedItem && (
 <>
 <div className="bg-white p-10 text-gray-900 relative border border-gray-200">
 ...
 <SheetTitle className="text-4xl font-black uppercase italic tracking-tighter text-white leading-none">
 {selectedItem.name}
 </SheetTitle>
 <SheetDescription className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-2">
 ...
 Rp{" "}
 {(
 Number(selectedItem.stockQty) *
 (selectedItem.valuations?.[0]?.movingAveragePrice ||
 ...
 <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-4 min-w-[150px]">
 ...
 {Number(selectedItem.stockQty).toLocaleString()}
 </p>
 </div>
 </div>
 </div>
 ...
 <TabsTrigger
 value="identity"
 className="rounded-lg font-black text-[10px] uppercase flex gap-2 data-[state=active]:bg-white" 
 >
 ...
 <div
 key={idx}
 className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-blue-200 transition-colors"
 >
 <div className="flex items-center gap-4">
 <div
 className={
 ...
 >
 <Box className={"h-4 w-4 " + clr} />
 </div>
 <p className="text-xs font-bold text-slate-700 uppercase">
 Internal Batch #{idx + 1}
 </p>
 </div>
 ...
 value={`${selectedItem.minLevel} ${selectedItem.unit}`}
 isAlert={
 Number(selectedItem.stockQty) <=
 ...
 <p className="text-center text-slate-400 text-xs font-bold uppercase italic py-10">
 ...
 <div
 key={log.id}
 className="flex items-center gap-6 p-4 border-b border-slate-50 group hover:bg-slate-50 transition-colors"
 >
 <div
 className={cn(
 "h-10 w-10 rounded-xl flex items-center justify-center shrink-0",
 ["INBOUND", "ADJUSTMENT_IN"].includes(
 log.type,
 )
 ...
 <Button className="flex-1 h-12 bg-gray-900 hover:bg-gray-800 text-white font-black uppercase text-[11px] rounded-xl tracking-widest border-none">
 <ArrowRightLeft className="mr-2 h-4 w-4" /> Stock Adjustment
 </Button>
 <Button
 variant="outline"
 className="flex-1 h-12 bg-white border-slate-200 text-slate-900 font-black uppercase text-[11px] rounded-xl tracking-widest"
 >
