    ...
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs font-sans animate-fade-in">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div>
            <label className="text-zinc-600 dark:text-zinc-400 font-medium block mb-1 flex items-center justify-between">
              <span className="flex items-center gap-1.5 font-bold text-amber-900 dark:text-amber-300">
                <span>🎯 Creative Brief & Notes</span>
              </span>
              <span className="text-[10px] text-zinc-400">(Instruksi Visual, Angle, atau USP Produk)</span>
            </label>
            <textarea
              rows={2}
              value={brief}
              onChange={(e) => setBrief(e.target.value)}
              placeholder="Contoh: Angle video edukasi perbandingan retinol vs bakuchiol untuk pemula, sertakan CTA konsultasi maklon..."
              className="w-full p-2.5 rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-amber-300/60 dark:border-zinc-700 text-[#37352f] dark:text-white outline-none focus:border-amber-500 leading-relaxed text-xs resize-y"
            />
          </div>
