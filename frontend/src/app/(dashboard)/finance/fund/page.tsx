  ...
  return (
    ...
    >
      <>
        ...
        <div hidden={activeTab !== "requests"} className="space-y-6 animate-fade-slide-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <DnaStatCard label="My Requests" value={myRequests.length} icon={<FileText className="text-slate-500" />} />
            <DnaStatCard label="Pending Approval" value={pendingCount} icon={<Lock className="text-amber-500" />} />
            <DnaStatCard label="Disbursed" value={paidCount} icon={<CheckCircle2 className="text-emerald-500" />} />
          </div>
          <DnaDataTableCard
            customToolbar={
        ...
        <div hidden={activeTab !== "approvals"} className="space-y-6 animate-fade-slide-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <DnaStatCard label="Total Pengajuan" value={allRequests.length} icon={<FileText className="text-blue-600" />} />
            <DnaStatCard label="Menunggu Approval" value={pendingApprovalCount} icon={<AlertTriangle className="text-amber-500" />} />
            <DnaStatCard label="Siap Cair" value={readyDisburseCount} icon={<ArrowRightCircle className="text-emerald-500" />} />
          </div>
          <DnaDataTableCard
            customToolbar={
              <div className="flex items-center justify-between w-full">
        ...
        <DialogContent className="sm:max-w-md bg-white rounded-2xl border border-slate-200 shadow-sm p-0 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            <div className="space-y-1.5">
              <label className="text-[8px] font-black uppercase tracking-wider text-slate-400">Department</label>
               <DnaSelect
                 ...
                 options={["MARKETING", "RND", "PRODUCTION", "SCM", "LEGAL", "CREATIVE", "HR", "FINANCE", "GENERAL"]}
               />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <DnaTextarea label="Reason" value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className="w-full h-24 bg-slate-50 border border-slate-200 rounded-xl p-4 font-medium text-xs resize-none focus:outline-none focus:border-blue-500 focus:bg-white transition-all" placeholder="Jelaskan tujuan pengajuan dana..." />
            </div>
          </div>
      ...
      <Dialog open={!!rejectId} onOpenChange={() => setRejectId(null)}>
        <DialogContent className="sm:max-w-md bg-white rounded-2xl border border-slate-200 shadow-sm p-0 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            <DnaTextarea label="Alasan Penolakan" value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} className="w-full h-24 bg-slate-50 border border-slate-200 rounded-xl p-4 font-medium text-xs resize-none focus:outline-none focus:border-rose-500 focus:bg-white transition-all" placeholder="Alasan penolakan pengajuan dana..." />
          </div>
          <DialogFooter className="p-6 pt-0 flex gap-2 justify-end">
