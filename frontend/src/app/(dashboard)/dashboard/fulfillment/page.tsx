  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <DnaCard className="lg:col-span-1 border-gray-200 bg-white h-fit">
           <DnaCardHeader>
              <DnaCardTitle className="text-sm font-black text-gray-600 uppercase tracking-[0.2em] flex items-center">
                 <Boxes className="mr-2 h-4 w-4 text-emerald-500" /> STOK BARANG JADI (FG)
              </DnaCardTitle>
              <DnaCardDescription className="text-[10px] uppercase font-bold text-zinc-600">Post-production inventory ledger</DnaCardDescription>
           </DnaCardHeader>
              ...
              {finishedGoods?.map(fg => (
                <div key={fg.id} className="p-4 bg-gray-50 border border-gray-200 rounded-sm hover:border-blue-500/50 transition-all cursor-default group">
                   <div className="flex justify-between items-start mb-1">
                      ...
                      <DnaBadge className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/30 text-[8px] font-black uppercase">READY</DnaBadge>
                   </div>
        ...
        {/* Shipment Tracker */}
        <DnaCard className="lg:col-span-2 border-gray-200 bg-white">
           <DnaCardHeader>
               <DnaCardTitle className="text-lg font-bold text-gray-900 uppercase tracking-tighter flex items-center">
              ...
              <div className="rounded-xl border border-gray-200 overflow-hidden">
                 <DnaTable>
                    <DnaTableHead className="bg-gray-50">
                       <DnaTableRow className="border-gray-200">
                          <DnaTh className="font-sans text-[10px] uppercase text-zinc-500">Surat Jalan</DnaTh>
                          <DnaTh className="font-sans text-[10px] uppercase text-zinc-500">Destination</DnaTh>
                          <DnaTh className="font-sans text-[10px] uppercase text-zinc-500">Status</DnaTh>
                          <DnaTh className="w-[150px]"></DnaTh>
                       </DnaTableRow>
                    </DnaTableHead>
                    <DnaTableBody>
                       {shipments?.map(ship => (
                          <DnaTableRow key={ship.id} className="border-gray-200 hover:bg-gray-50">
                             <DnaTd className="font-sans text-xs font-black text-blue-400 uppercase">{ship.id}</DnaTd>
                              <DnaTd className="text-xs font-bold text-gray-900 uppercase">{ship.so.lead.client_name}</DnaTd>
                             <DnaTd>
                                <DnaBadge className={`text-[9px] font-black uppercase rounded-none border ${
                             ...
                             <DnaTd className="text-right">
                                {ship.status === 'PACKING' && (
                                   <DnaButton size="sm" variant="outline" className="text-[9px] h-7 border-gray-200" onClick={() => updateShipmentStatus.mutate({ id: ship.id, status: 'SHIPPED' })}>
                                      SHIPPED
                                   </DnaButton>
                                )}
                                {ship.status === 'SHIPPED' && (
                                   <DnaButton size="sm" className="text-[9px] h-7 bg-emerald-500 text-black hover:bg-white" onClick={() => updateShipmentStatus.mutate({ id: ship.id, status: 'DELIVERED' })}>
                                      DELIVERED
                                   </DnaButton>
                                )}
      ...
      <DnaDialog open={isShipmentModalOpen} onOpenChange={setIsShipmentModalOpen}>
          <DnaDialogContent className="bg-white border-gray-200 text-gray-900 sm:max-w-md">
            <DnaDialogHeader>
               <DnaDialogTitle className="flex items-center text-2xl font-black italic uppercase tracking-tighter text-blue-500">
