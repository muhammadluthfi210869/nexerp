        ...
        return (
          <div className="grid grid-cols-3 gap-8 mb-6">
            ...
            <KpiCard
              ...
              targetPct={pendingPlans === 0 ? 100 : 0}
              icon={<Clock />}
            />
            <KpiCard label="Ready FRP" value={String(readyPlans)} targetPct={50} icon={<CheckCircle2 />} />
          </div>
        );
      })()}
        ...
        <Card className="lg:col-span-1 border-gray-200 bg-white h-fit">
          ...
          <CardContent className="space-y-4">
            {salesOrders?.map(so => (
              <div 
                key={so.id} 
                className="p-3 border border-gray-200 bg-black/40 rounded-sm hover:border-emerald-500/50 transition-all cursor-pointer group"
              ...
              >
                ...
                <p className="text-xs font-bold text-gray-900 group-hover:text-emerald-400 truncate uppercase">{so.lead?.client_name}</p>
                <div className="flex justify-between items-center mt-2 border-t border-gray-200 pt-2">
                    <p className="text-[10px] text-zinc-600">Qty: <span className="text-gray-900 font-sans">{so.quantity} Units</span></p>
                    <PlusCircle className="h-3 w-3 text-gray-400 group-hover:text-emerald-500" />
                </div>
              </div>
            ))}
            {(!salesOrders || salesOrders.length === 0) && (
              <div className="py-12 flex flex-col items-center justify-center opacity-20 bg-gray-50 rounded border border-dashed border-gray-200">
        ...
        <Card className="lg:col-span-3 border-gray-200 bg-white">
          <CardHeader>
             <div className="flex justify-between items-center">
                <div>
                    <CardTitle className="text-lg font-bold text-gray-900 uppercase tracking-tighter">FRP OPERATIONAL TRACKER</CardTitle>
            ...
            <form onSubmit={(e) => {
              ...
              generateFRP.mutate({
               ...
               <div className="space-y-4">
                  <div className="p-4 bg-gray-50 border border-gray-200 rounded-none space-y-2">
                     <p className="text-[10px] text-zinc-500 font-black uppercase text-center border-b border-gray-200 pb-2 mb-2">Contract Data Extract</p>
                     <div className="flex justify-between text-xs">
                        <span className="text-zinc-600 font-bold uppercase">Client:</span>
                         <span className="text-gray-900 font-sans uppercase">{selectedSO.lead?.client_name}</span>
                     </div>
                     <div className="flex justify-between text-xs">
                        <span className="text-zinc-600 font-bold uppercase">Batch Scale:</span>
                         <span className="text-gray-900 font-sans">{selectedSO.quantity} units</span>
                     </div>
                  </div>
                  ...
                  <div className="space-y-2">
                     ...
                     <Input 
                        ...
                        className="bg-white border-gray-200 h-12 font-sans text-lg focus:ring-emerald-500"
                        required
                     />
                  </div>
