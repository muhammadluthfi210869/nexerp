...
import { ChevronLeft, ChevronRight, Users, Eye, TrendingUp, BarChart3, FileText, Sparkles, Target, Music2, MonitorPlay, Globe, Megaphone } from "lucide-react";
import { DnaKpiGrid, DnaPageContainer, DnaPageHeader, DnaTabNav, DnaEmptyState } from "@/components/dna";
...
export default function BrandOverviewPage({ params }: { params: Promise<{ brand: string }> }) {
  ...
  return (
    <DnaPageContainer>
      ...
      <DnaTabNav
        tabs={[
          { id: "all", label: "All Overview", icon: BarChart3 },
            ...
            <>
              ...
              {report.storiesRecap && (
                <div className="mt-6 rounded-xl border border-slate-200 bg-white p-5">
                  <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Sparkles className="h-4 w-4" />Stories Recap</h3>
                  <div className="grid grid-cols-3 gap-4">
                    <div><div className="text-xs text-slate-500">Total Stories</div><div className="text-2xl font-bold">{report.storiesRecap.totalStoriesCreated}</div></div>
                    <div><div className="text-xs text-slate-500">Total Story Views</div><div className="text-2xl font-bold">{report.storiesRecap.totalStoryViews.toLocaleString("id-ID")}</div></div>
                    <div><div className="text-xs text-slate-500">Avg / Story</div><div className="text-2xl font-bold">{report.storiesRecap.avgViewsPerStory.toLocaleString("id-ID")}</div></div>
                  </div>
                </div>
              )}
      ...
      {tab === "tiktok" && <ChannelSection brandId={brandId} channel="TikTok" />}
      {tab === "youtube" && <ChannelSection brandId={brandId} channel="YouTube" />}
      {tab === "website" && <ChannelSection brandId={brandId} channel="Website" />}
      {tab === "ads" && <ChannelSection brandId={brandId} channel="Paid Ads" />}
    </DnaPageContainer>
  );
}
...
function WeeklyTab({ brandId, monthYear }: { brandId: string; monthYear: string }) {
  ...
  if (weeks.length === 0) return <DnaEmptyState title="Belum ada data weekly" />;
  return (
...
function FunnelTab() {
  return (
    <div className="mt-4 space-y-4">
      ...
      <p className="text-xs text-slate-500">Per-channel funnel detail landed in Phase 4 (deferred).</p>
    </div>
  );
}

function ChannelSection({ brandId, channel }: { brandId: string; channel: string }) {
  const [posts, setPosts] = useState<any[]>([]);
  useEffect(() => {
    marketingService.listPosts(mockViewer, { brandId, channel: channel as any, limit: 20 }).then((r: { items: any[] }) => setPosts(r.items));
  }, [brandId, channel]);
