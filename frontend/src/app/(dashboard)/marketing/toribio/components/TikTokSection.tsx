...
export function TikTokSection({ tracker, storiesKpi, weekly, bestContent }: Props) {
  if (!tracker || tracker.length === 0) {
    return (
      <div className="flex items-center justify-center h-32 rounded-[24px] border border-dashed border-gray-200 bg-gray-50/50">
        <p className="text-[10px] font-black uppercase tracking-[0.14em] text-gray-400">Belum ada data TikTok</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Key Metrics — 3 Dark Macro Cards */}
      ...
      <div>
        ...
        <WeeklyTable rows={weekly} platform="TikTok" />
      </div>
    </div>
  );
}
