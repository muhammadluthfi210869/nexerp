...
export function TaskWorkspace({ memberSlug }: { memberSlug: string }) {
  ...
  const member = members.data?.find((item) => slug(item.fullName) === memberSlug);
  ...
  return <DnaPageContainer className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6 lg:p-8">
    ...
    <DnaTabNav activeTab={memberSlug} onChange={(id) => router.push(`/marketing/management-task/${id}`)} tabs={[{ id: "overview", label: "Overview", icon: ListChecks }, { id: "my-tasks", label: "Task Saya", icon: Users }, ...(members.data ?? []).map((item) => ({ id: slug(item.fullName), label: item.fullName, icon: Users }))]} />
