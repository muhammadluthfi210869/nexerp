...
export async function KpiLeaderboard() {
  ...
  try {
    const res = await fetch(`${apiUrl}/v1/hr/kpi/leaderboard`, { next: { revalidate: 60 } });
    if (res.ok) {
  ...
  try {
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-black uppercase tracking-widest text-brand-black italic">
            📈 II. PERFORMANCE MATRIX (HUMAN CAPITAL)
          </h3>
        </div>
          ...
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-end">
                ...
                <span className="text-xl font-black text-brand-black tabular">
                  88.4 <span className="text-[10px] text-emerald-500 italic">+2.4%</span>
                </span>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {leaderboard.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-10 text-center text-slate-400 font-black uppercase text-[10px] italic tabular">
                      AWAITING HARVEST DATA...
                    </td>
                  </tr>
                ) : (
                  leaderboard.map((row: any, index: number) => (
                    <tr key={row.id || index} className="group hover:bg-slate-50/50 transition-all">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-brand-black uppercase tracking-tight italic">
                            {row.employee?.name || "Anonymous"}
                          </span>
                          <span className="text-[9px] font-bold text-slate-400 uppercase">
                            {row.employee?.roles?.[0]?.roleName || "UNASSIGNED"}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black text-emerald-600 tabular">
                            {(row.objectiveScore || 0).toFixed(1)}%
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black text-indigo-600 tabular">
                            {(row.subjectiveScore || 0).toFixed(1)}%
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className="text-sm font-black text-brand-black tabular tracking-tighter">
                          {(row.finalScore || 0).toFixed(1)}%
                        </span>
                      </td>
                    </tr>
                  ))
    ...
    return (
      <div className="bento-card p-10 text-slate-400 text-center uppercase font-black text-[10px] italic tabular border-dashed border-2">
        Leaderboard Offline - Harvesting Engine Reconnecting...
      </div>
    );
  }
}
