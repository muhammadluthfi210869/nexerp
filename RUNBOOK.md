...
```bash
bash scripts/rollback.sh 2  # try 2 deploys back
```
...
Common root causes:
- `Cannot find module './modules/X'` → missing module after merge → use bridge pattern instead of merge
...
| Rollback | `bash scripts/rollback.sh 1` |
| Snapshot DB | `bash scripts/db-snapshot.sh` |
| Restart backend only | `docker compose -p production-light restart backend` |
| Pull + rebuild | `git pull --ff-only && docker compose -p production-light up -d --build backend frontend` |
