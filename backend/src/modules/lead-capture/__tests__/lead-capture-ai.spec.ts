...
describe('LeadCaptureService — AI extraction (Fase 3.1)', () => {
  ...
  const parse = (content: string) =>
    (service as any).parseAndValidateExtraction(content);
  ...
  it('parse output JSON bersih dari LLM', () => {
    ...
    expect(out.fullName).toEqual({
      value: 'Andi',
      confidence: 0.9,
      source: 'saya Andi',
    });
    expect(out.moq).toEqual({
      value: 1000,
      confidence: 0.85,
      source: 'MOQ 1000 pcs',
    });
  ...
  it('parse saran stage (Fase 3.3) — hanya menerima WorkflowStatus yang valid', () => {
    const out = parse(
      JSON.stringify({
        stage: {
          ...
          confidence: 0.9,
          reason: 'customer minta sampel',
          source: 'minta sampel dong',
        },
      }),
    );
    ...
    const bad = parse(
      JSON.stringify({
        stage: { stage: 'TIDAK_ADA_STAGE', confidence: 0.9, reason: 'x' },
      }),
    );
...
describe('LeadCaptureService — confirmAttribute (Fase 3.2, Option B)', () => {
  ...
  it('konfirmasi fullName → ikut menulis kolom lead fullName (first-class)', async () => {
    prismaMock.leadAttribute.findFirst.mockResolvedValue({
      id: 'a1',
      key: 'fullName',
      value: 'Andi',
      confirmed: false,
    });
    prismaMock.leadAttribute.update.mockResolvedValue({
      id: 'a1',
      key: 'fullName',
      value: 'Andi',
      confirmed: true,
    ...
    expect(prismaMock.leadCapture.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ fullName: 'Andi' }),
      }),
    );
    // setelah semua terkonfirmasi → aiStatus CONFIRMED
    expect(prismaMock.leadCapture.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ aiStatus: 'CONFIRMED' }),
      }),
    );
  });
