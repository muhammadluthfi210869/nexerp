...
const BACKEND_ENDPOINTS = [
  ...
  },
  {
    ...
    frontendFields: ['id', 'name', 'isActive', 'costPerHour'],
  },
  {
...
async function runAudit() {
  ...
  for (const ep of BACKEND_ENDPOINTS) {
    try {
      ...
      const missingFromBackend = frontendFields.filter(
        (f) => !backendFields.includes(f),
      );
      const extraInBackend = backendFields.filter(
        (f) => !frontendFields.includes(f),
      );
  ...
  for (const r of results) {
    ...
    if (r.missingFromBackend.length > 0) {
      console.log(
        `   Missing from backend: ${r.missingFromBackend.join(', ')}`,
      );
    }
    if (r.extraInBackend.length > 0) {
      console.log(
        `   Extra in backend (sample): ${r.extraInBackend.slice(0, 5).join(', ')}...`,
      );
    }
  }
  ...
  if (mismatches.length > 0) {
    ...
    for (const m of mismatches) {
      for (const field of m.missingFromBackend) {
        console.log(
          `[FIX] ${m.endpoint}: Frontend expects "${field}" but backend doesn't return it.`,
        );
        console.log(
          `      → Action: Either fix frontend to use correct field, or add "${field}" to backend response.\n`,
        );
      }
    }
  }
