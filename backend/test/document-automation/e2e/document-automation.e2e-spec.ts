...
describe('Document Automation — E2E API', () => {
  ...
  runTest(
    'D1: POST /document-automation/generate/quotation/:leadId',
    async () => {
  ...
  runTest(
    'D2: POST /document-automation/generate/dp-invoice/:salesOrderId',
    async () => {
      ...
      const soRes = await request(app.getHttpServer())
        ...
        .send({
          leadId,
          sampleId:
            (await prisma.sampleRequest.findFirst({ where: { leadId } }))?.id ||
            '',
  ...
  runTest(
    'D4: GET /document-automation/drafts?documentType=INVOICE_DP — filter by type',
    async () => {
      const res = await request(app.getHttpServer())
  ...
  runTest(
    'D6: GET /document-automation/drafts/:id — get draft detail',
    async () => {
  ...
  runTest(
    'D7: PATCH /document-automation/drafts/:id — update draft',
    async () => {
      const createRes = await request(app.getHttpServer())
        .post(`/document-automation/generate/quotation/${leadId}`)
        .expect(201);
  ...
  runTest(
    'D8: POST /document-automation/drafts/:id/approve — approve draft',
    async () => {
      const createRes = await request(app.getHttpServer())
        .post(`/document-automation/generate/quotation/${leadId}`)
        .expect(201);
  ...
  runTest(
    'D9: POST /document-automation/drafts/:id/reject — reject draft',
    async () => {
      const createRes = await request(app.getHttpServer())
        .post(`/document-automation/generate/quotation/${leadId}`)
        .expect(201);
  ...
  runTest(
    'D10: GET /document-automation/drafts/invalid-id — 404 error',
    async () => {
  ...
  runTest(
    'D11: POST /document-automation/pdf — generate PDF directly',
    async () => {
      const res = await request(app.getHttpServer())
  ...
  runTest(
    'D12: GET /document-automation/drafts/:id/pdf — download draft PDF',
    async () => {
      const createRes = await request(app.getHttpServer())
        .post(`/document-automation/generate/quotation/${leadId}`)
        .expect(201);
