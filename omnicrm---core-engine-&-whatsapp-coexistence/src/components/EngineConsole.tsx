...
export const EngineConsole: React.FC<EngineConsoleProps> = ({
  ...
  const defaultPayloads: Record<AvailableTool, string> = {
    ...
    move_lead_stage: JSON.stringify(
      {
        lead_id: state.leads[0]?.id || 'lead_28981912',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex items-center justify-between">
        ...
        <button
          onClick={onClearLogs}
          className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 text-xs font-bold transition-colors cursor-pointer"
        >
          Bersihkan Log
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* RPC Executor Form */}
        <div className="lg:col-span-6 bg-white border border-slate-200 rounded-2xl p-4 shadow-xs space-y-4">
          ...
          <form onSubmit={handleExecute} className="space-y-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-900 font-mono font-semibold focus:outline-none focus:ring-1 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 text-xs text-slate-900 font-mono focus:outline-none focus:ring-1 focus:ring-blue-500 focus:bg-white resize-none"
              />
              {jsonError && (
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-sm flex items-center justify-center gap-2 cursor-pointer transition-colors"
            >
        ...
        <div className="lg:col-span-6 bg-white border border-slate-200 rounded-2xl p-4 shadow-xs space-y-4 flex flex-col">
          <div className="flex items-center justify-between pb-2 border-b border-slate-200">
            <div className="flex items-center gap-2">
